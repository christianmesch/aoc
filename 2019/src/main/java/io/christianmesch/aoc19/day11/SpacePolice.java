package io.christianmesch.aoc19.day11;
...
public class SpacePolice {
    ...
    private final Map<Coordinate, Long> panels = new HashMap<>();
    private String orientation = "U";

    public static void main(String... args) {
        SpacePolice day = new SpacePolice();
    ...
    private void part2() throws InterruptedException {
        orientation = "U";
        panels.clear();
        panels.put(new Coordinate(0, 0), 1L);

        run();

        final int minX = panels.keySet().stream().map(c -> c.x).min(Integer::compareTo).orElse(-1);
        final int maxX = panels.keySet().stream().map(c -> c.x).max(Integer::compareTo).orElse(-1) - minX;
        final int minY = panels.keySet().stream().map(c -> c.y).min(Integer::compareTo).orElse(-1);
        final int maxY = panels.keySet().stream().map(c -> c.y).max(Integer::compareTo).orElse(-1) - minY;

        Map<Coordinate, Long> normalized = panels.entrySet().stream()
                .map(e -> Map.entry(new Coordinate(e.getKey().x - minX, e.getKey().y - minY), e.getValue()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

        for (int y = maxY; y >= 0; y--) {
            for (int x = 0; x <= maxX; x++) {
                System.out.print(normalized.getOrDefault(new Coordinate(x, y), 0L) == 1L ? "#" : " ");
            }
            System.out.println("");
        }
    }
    ...
    private void run() throws InterruptedException {
        ...
        while(robot.isAlive()) {
            long panel = panels.getOrDefault(coordinate, 0L);
            input.putLast(panel);

            for(int i = 0; i < 2; i++) {
                Long o = output.pollFirst(100, TimeUnit.MILLISECONDS);
                if(o == null) break;

                if(i % 2 == 0) {
                    panels.put(coordinate, o);
                } else {
                    coordinate = updateCoordinate(coordinate, o);
                }
            }
        }
    }

    private Coordinate updateCoordinate(Coordinate coord, long turn) {
        ...
        return switch (orientation) {
            case "U" -> new Coordinate(coord.x, coord.y + 1);
            case "R" -> new Coordinate(coord.x + 1, coord.y);
            case "D" -> new Coordinate(coord.x, coord.y - 1);
            case "L" -> new Coordinate(coord.x - 1, coord.y);
            default -> throw new IllegalArgumentException("Unknown orientation: " + orientation);
        };
    }
